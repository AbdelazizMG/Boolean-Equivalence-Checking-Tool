...
def findParent(node,current_add,parent_add) -> None:
    global Parent_Add
    if (node is None):
        return None
    # If current node is
    ...
    if (node == current_add):  
        ...
        # Current node is now
        # the new parent
        findParent(node.left,current_add,node)
        findParent(node.right,current_add,node)
...
def Remove_Nodes(list, Reduced_List):
    global Parent_Add
    for i in range(0,len(list)):
        if(list[i].label==0):
            findParent(list[-1], list[i], -1)
            if(Parent_Add.left==list[i]):
                Parent_Add.left= Reduced_List[0]
            else:
                Parent_Add.right= Reduced_List[0]
            list[i].label=-1
        elif(list[i].label==1):
            findParent(list[-1], list[i], -1)
            if(Parent_Add.left==list[i]):
                Parent_Add.left= Reduced_List[1]
            else:
                Parent_Add.right= Reduced_List[1]
            list[i].label=-1
        else:
            if(list[i].left.label==list[i].right.label) and (list[i].left.label!=-1) and (list[i].right.label!=-1):
                ...
                #print(i)
                #print(Parent_Add)
                if( (Parent_Add != None) and (Parent_Add != -1)):
                    if(Parent_Add.left==list[i]):
                        #print(Parent_Add)
                        ...
                        Parent_Add.left= list[i].right
                    else:
                        #list[i].right = list[i].left
                        Parent_Add.right= list[i].right
            ...
            if(list[j+index].label!=-1):
                for k in range(init, pow(2, i)+init):
                    if(list[index+j] != list[index+k]) and (list[index+j].label==list[index+k].label):
                        ...
                        findParent(list[-1], list[index+k], -1)
                        ...
                        if(Parent_Add.left==list[index+k]):
                            Parent_Add.left= list[index+j]
                        else:
                            Parent_Add.right= list[index+j]
        index=index+pow(2, i)
    #print("****** AFTER *******")
    #for i in range(0,len(list)):
    #    print("\n")
...
def Equivelance_Checker(List1,List2):
...
#print("****************")
#print(Equivelance_Checker(list1,list2))
...
for  i in range(0,len(list1)):
    ...
    if(list1[i].left != None):
        if(type(list1[i].left.data)==type("y") and len(list1[i].left.data)==1):
            list1[i].left.data=list1[i].left.data+"1"
        ...
        if(type(list1[i].right.data)==type("y") and len(list1[i].right.data)==1):
            list1[i].right.data=list1[i].right.data+"2"
